...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submitted'])) {
        ...
        $validation = validate_delivery($_POST['r_f_no_id'], $_POST['r_kg'], $_POST['r_dt']);
        if ($validation['valid']) {
            $datetime = strtotime($_POST['r_dt']);
            $mysqldate = date("Y-m-d H:i:s", $datetime);
            $sql = "UPDATE `delivery` SET  `r_f_no` =  '{$_POST['r_f_no_id']}' ,  `r_kg` =  '{$_POST['r_kg']}' ,  `r_dt` =  '{$mysqldate}' ,  `r_deliverer` =  '{$_POST['r_deliverer']}'   WHERE `id` = '$id' ";
