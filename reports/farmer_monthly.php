    ...
    <h5 id="farmer_details">
     <?php
        if (isset($_REQUEST['farmer']) && $_REQUEST['farmer'] != '') {
            ...
            $result = mysqli_query($conn,"SELECT * FROM `delivery` WHERE r_f_no_id=$f_no $start $end") or trigger_error(mysqli_error($conn));
            // echo "SELECT * FROM `delivery` WHERE r_f_no_id=$f_no $start $end";
